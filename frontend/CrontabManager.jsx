                ...
                <table className="w-full text-left text-xs border-collapse">
                    <thead className="text-slate-500 font-semibold border-b border-[#30363d]">
                        <tr>
                            ...
                            <th className="py-2 px-2 text-right">Log File</th>
                        </tr>
                    </thead>
                    <tbody className="text-slate-300">
                        {history.map((run) => (
                            ...
                            >
                                ...
                                <td className="py-2.5 px-2 text-right font-mono flex items-center justify-end gap-2">
                                    ...
                                    <span className="text-slate-500 text-xs hover:text-slate-300 transition-colors">{run.filename}</span>
                                </td>
                            </tr>
                        ))}
                    </tbody>
...
export default function CrontabManager() {
    ...
    const getJobIdFromUrl = () => {
        ...
        const match = path.match(/\/crontab\/job\/(\d+)/);
        return match ? parseInt(match[1]) : null;
    };

    const initialDetailId = getJobIdFromUrl();
    const [viewingDetailId, setViewingDetailId] = useState(initialDetailId);
    const [selectedJobId, setSelectedJobId] = useState(initialDetailId);
    ...
    // Load history if directly landing on detail page
    useEffect(() => {
        if (viewingDetailId && !mockHistories[viewingDetailId]) {
            setMockHistories(prev => ({ ...prev, [viewingDetailId]: generateMockHistory(viewingDetailId) }));
        }
    }, [viewingDetailId]);
    ...
    const handleViewDetail = (e, id) => {
        e.stopPropagation();
        // Navigate to new URL
        window.history.pushState({}, '', `/crontab/job/${id}`);
        setViewingDetailId(id);
        setSelectedJobId(id);
    ...
    const handleBackToDashboard = () => {
        window.history.pushState({}, '', '/crontab');
    ...
    const generateMockHistory = (jobId) => {
        return Array.from({ length: 8 }).map((_, i) => {
            ...
            date.setHours(Math.floor(Math.random() * 23), Math.floor(Math.random() * 59), Math.floor(Math.random() * 59));

            // Format for display: YYYY-MM-DD HH:MM:SS
            const runAt = date.toLocaleString('sv-SE');
            ...
            const trigger = Math.random() > 0.3 ? 'SCH' : 'MAN';
            const triggerFull = trigger === 'SCH' ? 'SCHEDULE' : 'MANUAL';
            const filename = `${yyyy}${mm}${dd}_${HH}${MM}${SS}_${trigger}.log`;

            const logLines = [];
            logLines.push(`[${runAt}] [INFO] Initializing environment for Job #${jobId}...`);
            for (let j = 0; j < 50; j++) {
                const time = new Date(date.getTime() + j * 100).toISOString().split('T')[1].slice(0, -1);
                logLines.push(`[${runAt.split(' ')[0]} ${time}] [DEBUG] Processing data chunk ...`);
            }

            if (!isSuccess) {
                logLines.push(`[${runAt}] [ERROR] Connection timed out.`);
                logLines.push(`[${runAt}] [FATAL] Process terminated.`);
            } else {
                logLines.push(`[${runAt}] [INFO] Job finished successfully.`);
            }
                                        ...
                                        >
                                            ...
                                            <td className="py-2 px-3 max-w-lg">
                                                <div className="flex flex-col gap-0.5">
                                                    ...
                                                    <div className="text-slate-300 whitespace-pre-wrap break-all font-medium font-mono text-[11px]" title={job.command}>
                                                        {isEditing ? (
                                                            <textarea
                                                    ...
                                                    <div className="flex justify-end gap-1">
                                                        {job.status === 'running' ? (
                                                            ...
                                                            <button
                                                                ...
                                                                className={`p-1.5 rounded border flex items-center gap-1 transition-colors ${!job.enabled ? 'bg-transparent border-slate-800 text-slate-700 cursor-not-allowed' : 'bg-transparent border-emerald-900/50 text-emerald-500 hover:bg-emerald-900/20 hover:border-emerald-500/50'}`}
                                                                title="Run Now"
                                                            >
                                                                <Play size={12} className={job.status === 'running' ? '' : 'fill-current'} />
                                                            </button>
                                                        )}
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-1.5 rounded border border-blue-900/50 text-blue-500 bg-transparent hover:bg-blue-900/20 hover:border-blue-500/50 transition-colors"
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-1.5 rounded border border-amber-900/50 text-amber-500 bg-transparent hover:bg-amber-900/20 hover:border-amber-500/50 transition-colors"
                ...
                <div className="h-48 bg-[#010409] border-t border-[#30363d] flex flex-col shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.5)] shrink-0 z-10">
                    <div className="h-8 bg-[#161b22] border-b border-[#30363d] px-3 flex items-center justify-between text-xs">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setLogs([])} className="bg-transparent border-0 hover:text-white text-slate-600">Clear</button>
                            <button onClick={() => setSelectedJobId(null)} className={`bg-transparent border-0 hover:text-white ${selectedJobId ? 'text-blue-400' : 'text-slate-600'}`}>
